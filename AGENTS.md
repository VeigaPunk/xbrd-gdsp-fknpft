...
|-------|-------|------|-----------------|
| **the-judge** | **opus 4.7 · xhigh** (orchestrator exception — user directive 2026-04-17: every other teammate is sonnet-medium, judge stays opus for intermediation depth) | Orchestrator and arbiter. Names axes, dispatches specialists, applies Pareto filter, drafts implementation. Top of the stack. | In-session (spawns others, never spawned) |
...
| **reviewer** | sonnet · medium | Surgical code reviewer. Finds the bug that ships to prod. Delegates to Codex always. | `xask -R codex` (review lane — full gpt-5.4) |
...
| **the-revenger** | sonnet · medium | Reverse engineering specialist. Maps behavior, infers intent, reproduces functionality. Godspeeded by default. 4-phase RECON/PROBE/MODEL/BUILD protocol. | `xask -R codex` for surface enum (review lane), direct recon |
