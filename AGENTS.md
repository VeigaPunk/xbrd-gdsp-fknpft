...
| **labrat** | sonnet · medium | Expendable single-shot probe. Tests one hypothesis cheap and fast. State nuked on despawn. | `xask --spark --gs codex` (default), `xask --effort high codex` when spark is insufficient |
| **connector** | sonnet · medium | Cross-axis pattern matcher. Sees the whole table, calls out unusual connections and second-order effects. Breadth over depth. | `xask --effort medium codex` (no `--gs`; avoids double-godspeed frame on pontification-prone lane), `advisor()` for reasoning |
...
| **sentinel** | sonnet · medium | Security auditor. Attacker-minded — hunts vulnerabilities, injection vectors, insecure configs, privilege escalation. Full tool access for scanning and remediation. | `xask --gpt55 --gs -e low codex` for exploit analysis, `xask --effort medium --gs codex` for CVE prior art, `advisor()` for multi-hop chains |
| **critic** | sonnet · medium + Layer-0 `Skill('heuer-planning')` | Approach-level adversarial reviewer. Challenges design decisions, architectural assumptions, and strategy choices. Distinct from reviewer (code bugs) and sentinel (security). | `xask --gpt55 --gs -e low codex` for deep design review, `xask --effort medium --gs codex` for alternatives |
