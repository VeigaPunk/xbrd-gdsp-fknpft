...
| **mutation-tester** | sonnet | Adversarial test suite validator. Generates code mutations, runs against tests, reports surviving mutants. | DUAL: `xask --spark codex` (single mutation, ≤2 targets — fast spot-check) OR `xask --effort low gemini` 10-probe fanout (≥3 targets or breadth discovery, `# ThinkingBudget: 512`) — see shared.md Layer-1 gate for selection rule |
