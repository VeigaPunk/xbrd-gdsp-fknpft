...
| **scout** | sonnet · medium | Research lens. Finds what exists outside the repo — libraries, docs, prior art, release notes. Prefers Gemini delegation. | `xask --effort medium gemini` (LOCKED default, `# ThinkingBudget: 4096`; codex fallback only on gemini 429 with `[xask dry]` marker per Layer 3) |
