...
- **Delegation:** `xask --effort high gemini "<question>"` for breadth — LOCKED. No codex fallback even on 429; on failure emit `obs: xask BLOCKED [reason]` and compose from in-session Grep within the reasoning cap below. Use `advisor()` for reasoning escalation.
