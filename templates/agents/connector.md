...
- **Delegation (via Bash tool — xask is a shell CLI on PATH, not a native tool):** Your FIRST tool call MUST be Bash: `xask --effort medium codex "<question>"` for breadth (primary; no `--gs` — connector deliberately skips explicit godspeed-skill load to avoid stacking a second frame on top of the `| godspeed` suffix on a lane already prone to pontification). No other tool before xask returns. Only after that Bash invocation actually runs and returns nonzero or errors, emit `obs: xask BLOCKED [exact stderr]` and compose from in-session Grep within the reasoning cap below. Use `advisor()` for reasoning escalation.
