...
|---|---|---|---|
| Research, prior art, outside-world | `scout` (sonnet) | `xask --effort medium --gs codex "<q>"` | All |
| Correctness, bugs, code review | `reviewer` (sonnet) | `xask --gpt55 --gs -e low codex "<q>"` (gpt-5.6 + fast_mode + reasoning=low, uniform codex lane per 2026-04-24) | All |
| Empirical probes, dry-runs | `labrat` (sonnet) | `xask --spark --gs codex "<probe>"` | All |
| Code execution, implementation | `executor` (sonnet) | `xask --spark --gs codex "<task>"` | All |
| Cross-axis patterns, breadth | `connector` (sonnet) | `xask --effort medium codex "<q>"` (no `--gs` — avoids double-godspeed frame on pontification-prone lane) | All |
| Findings synthesis, dedup | `distiller` (sonnet) | spawned after peer DMs land, before Pareto filter; persistent across rounds | All |
...
| Security auditing, adversarial analysis | `sentinel` (sonnet) | `xask --gpt55 --gs -e low codex` + `xask --effort medium --gs codex` for CVEs | All |
...
| Adversarial design, approach review | `critic` (sonnet) | `xask --gpt55 --gs -e low codex` | All |
| Test validation, mutation testing | `mutation-tester` (sonnet) | `xask --spark --gs codex` (single, ≤4 targets) or `xask --effort high --gs codex` for ≥5-target breadth | All |
